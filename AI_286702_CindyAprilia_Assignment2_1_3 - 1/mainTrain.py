...
class MainClass():
        ...
        while (playing==True) :
            ...
            if (str(choose1) == "1") :
                ...
                if (choose2 == "M"):
                    ...
                    if (choose3 == "Y"):
                        train_agent('M', n_games=Iterates, games_per_checkpoint=1, model_save_path=(CongklakTrainingAgent.MODEL_FOLDER() + "/" + "mancala_agent1.pkl"), depth=int(RECOMMENDED_LEVEL), cutVariance=CUT_VARIANCE)
                    else:
                        choose4 = input("Which level do you want it to be?\n")
                        train_agent('M', n_games=Iterates, games_per_checkpoint=1, model_save_path=(CongklakTrainingAgent.MODEL_FOLDER() + "/" + "mancala_agent1.pkl"), depth=int(str(choose4)), cutVariance=CUT_VARIANCE)
                else:
                    train_agent('R', n_games=Iterates, games_per_checkpoint=1, model_save_path=(CongklakTrainingAgent.MODEL_FOLDER() + "/" + "mancala_agent1.pkl"), cutVariance=CUT_VARIANCE)
            elif (str(choose1) == "3") :
                #play
                Iterates = int(input("How many times to test? "))
                
                for iterate in range (Iterates):
                    self.CompareTrained('MR', 'MR')
    ...
    def CompareTrained(self, agentType1='M', agentType2='M'):
        ...
        if "Draw" not in winner : 
            ...
            winner_Name = 0
            winning_Type = "Draw"
        CongklakSearchingAgent.saveTestingData('CompareTrainedResult.csv', [[CongklakBoard.NUMBER_OF_HOLE(), CongklakBoard.NUMBER_OF_BEADS(), CUT_VARIANCE, 'RL', agentType1, agentType2, winner_Name, winning_Type, steps_taken, performance_time]])
